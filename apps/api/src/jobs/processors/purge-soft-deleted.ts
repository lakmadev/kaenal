 ...
 * with its singular audit `entityKind`.
 *
 * Deliberately excluded:
 *  - Access/identity (`memberships`, `notification_prefs`, `sessions`) — the
 *    DSAR/offboarding lifecycle owns those (07 §5), not record retention.
 *  - `exports` — generated artifacts, cleaned up with their S3 objects by the
 *    files pipeline, not a quality record.
 *  - `documents` / `files` — their dependents (`document_versions` has no
 *    independent `deleted_at`; `signatures`; the S3 object) need a considered
 *    cascade + object-store design; tracked as a follow-up rather than purged
 *    half-safely here.
 */
...
export async function purgeSoftDeletedForTenant(
  ...
  deps: { now?: Date; retentionDays?: number } = {},
  ...
  return withTenant(payload.tenantId, null, async (tx) => {
    ...
    if (hasTenantWideHold(scopes)) return { purged: 0 };

    let purged = 0;
    for (const { table, entityKind } of PURGE_ORDER) {
      const { rows } = await tx.query<{ id: string }>(
        // `table` is from the fixed PURGE_ORDER list, never user input.
      ...
      for (const { id } of rows) {
        if (isBlockedByHolds(scopes, { entityKind, entityId: id })) continue;
        if (await purgeRow(tx, payload.tenantId, table, entityKind, id)) purged += 1;
      }
    }
...
/**
 * Delete one row inside a SAVEPOINT, writing its `purged` audit event in the
 * same step. Returns true if it was purged, false if a still-live reference
 * (FK RESTRICT) blocked it — that row simply waits for a later run.
 */
