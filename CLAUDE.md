...
## Ground truth
- `implementation/` is the canonical spec: README (phases + rules), 01-Architecture, 02-Database, 03-API, 04-Web, 05-Mobile, 06-Jobs/AI, 07-Security, 08-Testing, 09-Integrations.
- `project_brain/` holds prior session notes. On conflict, `implementation/` wins — log the conflict in PROGRESS.md.
- `Kaenal.html` + `src/*.jsx` + `styles/tokens.css` are the VISUAL spec only. Never copy prototype code into the codebase; recreate screens per `implementation/04-WEB-APP.md`.
...
## Stack (details in implementation/reference/TECH_STACK.md)
...
- `pnpm db:migrate` / `pnpm db:check` (RLS schema lint) / `pnpm provision-tenant --slug acme --name "Acme" --model shared`
- `pnpm test` (unit) / `pnpm test:rls` (tenancy suite) / `pnpm e2e`
